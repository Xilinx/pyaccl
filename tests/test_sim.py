...
@pytest.fixture(scope="module", params=[{"rxbuf_size":16*1024, "tcp":True}])
...
def test_copy(cclo_inst, count, dt):
    op_buf, _, res_buf = get_buffers(count, dt, dt, dt, cclo_inst)
    cclo_inst.copy(op_buf, res_buf, count)
    assert np.isclose(op_buf.data, res_buf.data, atol=1e-02).all()

@pytest.mark.parametrize("count", [1, 16, 21, 128])
@pytest.mark.parametrize("dt", [np.float32, np.float16, np.int32, np.int64])
def test_combine(cclo_inst, count, dt):
    op0_buf, op1_buf, res_buf = get_buffers(count, dt, dt, dt, cclo_inst)
    cclo_inst.combine(count, ACCLReduceFunctions.SUM, op0_buf, op1_buf, res_buf)
    assert np.isclose(op0_buf.data+op1_buf.data, res_buf.data, atol=1e-02).all()
...
def test_sendrecv(cclo_inst, world_size, local_rank, count, dt):
    ...
    assert np.isclose(op_buf.data, res_buf.data).all()
...
def test_scatter(cclo_inst, world_size, local_rank, root, count, dt):
    op_buf, _, res_buf = get_buffers(count*world_size, dt, dt, dt, cclo_inst)
    op_buf[:] = [1.0*i for i in range(op_buf.size)]
    cclo_inst.scatter(op_buf, res_buf, count, root=root)
    assert np.isclose(op_buf.data[local_rank*count:(local_rank+1)*count], res_buf.data[0:count]).all()
...
def test_gather(cclo_inst, world_size, local_rank, root, count, dt):
    ...
    if local_rank == root:
        for i in range(world_size):
            assert np.isclose(res_buf.data[i*count:(i+1)*count], [1.0*(i+j) for j in range(count)]).all()
...
def test_reduce(cclo_inst, world_size, local_rank, root, count, func, dt):
    ...
    if local_rank == root:
        assert np.isclose(res_buf.data, sum(range(world_size+1))*op_buf.data).all()
...
def test_reduce_scatter(cclo_inst, world_size, local_rank, count, func, dt):
    op_buf, _, res_buf = get_buffers(world_size*count, dt, dt, dt, cclo_inst)
    op_buf[:] = [1.0*i for i in range(op_buf.size)]
    cclo_inst.reduce_scatter(op_buf, res_buf, count, func)
    full_reduce_result = world_size*op_buf.data
    assert np.isclose(res_buf.data[0:count], full_reduce_result[local_rank*count:(local_rank+1)*count]).all()
...
def test_allreduce(cclo_inst, world_size, count, func, dt):
    ...
    full_reduce_result = world_size*op_buf.data
    assert np.isclose(res_buf.data, full_reduce_result).all()
...
def test_multicomm(cclo_inst, world_size, local_rank):
    ...
    if local_rank == 0:
        ...
        assert np.isclose(res_buf.data, op_buf.data).all()
    elif local_rank == 2:
    ...
    assert np.isclose(res_buf.data, expected_allreduce_result).all()
...
def test_multicomm_allgather(cclo_inst, world_size, local_rank, count):
    ...
    for i, g in enumerate(new_group):
        assert np.isclose(res_buf.data[i*count:(i+1)*count], [1.0*(g+j) for j in range(count)]).all()

    op_buf[:] = [1.0 * local_rank for i in range(op_buf.size)]
    cclo_inst.bcast(op_buf, count, root=0)
    if local_rank != 0:
        assert np.isclose(op_buf.data, [0.0 for i in range(op_buf.size)]).all()
...
def test_eth_compression(cclo_inst, world_size, local_rank, count):
    ...
    if local_rank == 0:
        ...
        assert np.isclose(op_buf.data.astype(np.float16).astype(np.float32), res_buf.data, rtol=1e-02, atol=1e-02).all()
    ...
    if local_rank == 0:
        assert np.isclose(res_buf.data, world_size*op_buf.data, rtol=1e-02, atol=1e-02).all()
    ...
    if local_rank > 0:
        assert np.isclose(op_buf.data[local_rank*count:(local_rank+1)*count].astype(np.float16).astype(np.float32), res_buf.data[0:count], rtol=1e-02, atol=1e-02).all()

    cclo_inst.gather(op_buf, res_buf, count, 0, compress_dtype=np.dtype('float16'))
    if local_rank == 0:
        for i in range(world_size):
            assert np.isclose(op_buf.data[0:count].astype(np.float16).astype(np.float32), res_buf.data[i*count:(i+1)*count], rtol=1e-02, atol=1e-02).all()
    ...
    assert np.isclose(expected_res.astype(np.float16).astype(np.float32), res_buf.data[0:count], rtol=1e-02, atol=1e-02).all()
